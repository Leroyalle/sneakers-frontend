import Styles from './NavUser.module.scss';
export const NavUser = ({ openDrawer }) => {
  return (
    <nav className={Styles['navigation-user']}>
      <ul className={Styles['navigation-user__list']}>
        <li className={Styles['navigation-user__item']} onClick={openDrawer}>
          <span className="visually-hidden">Корзина</span>
          <svg className={Styles['navigation-user__icon']} width="20" height="20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M7.545 18.182a.818.818 0 1 0 0-1.637.818.818 0 0 0 0 1.637ZM16.546 18.182a.818.818 0 1 0 0-1.637.818.818 0 0 0 0 1.637ZM1 1h3.273l2.192 10.956a1.636 1.636 0 0 0 1.637 1.317h7.953a1.636 1.636 0 0 0 1.636-1.317L19 5.09H5.09"
              stroke="#9B9B9B"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>
          <span className="navigation-user__text">1205 руб.</span>
        </li>
        <li className={Styles['navigation-user__item']}>
          <span className="visually-hidden">Избранное</span>
          <svg className={Styles['navigation-user__icon']} width="19" height="17" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clipPath="url(#a)">
              <path
                d="M12.954 0c1.397 0 2.57.471 3.521 1.414.95.942 1.425 2.093 1.425 3.45 0 .666-.14 1.338-.419 2.017-.28.68-.587 1.282-.922 1.809-.335.527-.901 1.206-1.698 2.037a56.74 56.74 0 0 1-2.012 2.017c-.545.513-1.418 1.296-2.62 2.35l-1.3 1.163-1.3-1.122c-1.173-1.081-2.04-1.878-2.598-2.391a45.802 45.802 0 0 1-2.034-2.017C2.201 9.896 1.635 9.217 1.3 8.69a10.437 10.437 0 0 1-.902-1.809A5.509 5.509 0 0 1 0 4.865c0-1.359.475-2.509 1.425-3.451C2.375.47 3.535 0 4.905 0c1.62 0 2.962.624 4.024 1.871C9.991.624 11.333 0 12.954 0Zm-3.94 13.804c1.369-1.22 2.368-2.127 2.997-2.723.628-.596 1.32-1.31 2.075-2.141.754-.832 1.278-1.56 1.572-2.183.293-.624.44-1.255.44-1.892 0-.887-.3-1.622-.901-2.204-.601-.582-1.349-.873-2.243-.873-.671 0-1.307.194-1.908.582-.6.388-1.027.887-1.278 1.497H8.09c-.224-.61-.636-1.109-1.237-1.497a3.525 3.525 0 0 0-1.95-.582c-.894 0-1.634.29-2.221.873-.587.582-.88 1.317-.88 2.204 0 .637.14 1.268.419 1.892.28.623.803 1.35 1.572 2.183a45.845 45.845 0 0 0 2.096 2.14c.629.597 1.614 1.505 2.955 2.724l.084.083.084-.083Z"
                fill="#9B9B9B"
              />
            </g>
            <defs>
              <clipPath id="a">
                <path fill="#fff" d="M0 0h18.11v16.299H0z" />
              </clipPath>
            </defs>
          </svg>
          <span className="navigation-user__text">Избранное</span>
        </li>
        <li className={Styles['navigation-user__item']}>
          <span className="visually-hidden">Профиль</span>
          <svg className={Styles['navigation-user__icon']} width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M0 9C0 4.1211 4.1211 0 9 0C13.8789 0 18 4.1211 18 9C18 11.871 16.5672 14.4702 14.4 16.1316V16.2H14.3082C12.807 17.3205 10.9683 18 9 18C7.0317 18 5.193 17.3205 3.6918 16.2H3.6V16.1316C1.4328 14.4702 0 11.8701 0 9ZM6.41112 13.7124C5.93239 14.0975 5.59923 14.6344 5.4666 15.2343C6.5178 15.8436 7.7256 16.2 9 16.2C10.2744 16.2 11.4822 15.8436 12.5334 15.2343C12.4006 14.6344 12.0674 14.0977 11.5887 13.7126C11.11 13.3275 10.5144 13.1169 9.9 13.1157H8.1C7.48557 13.1168 6.88986 13.3272 6.41112 13.7124ZM12.391 12.0705C13.129 12.5617 13.7057 13.2596 14.049 14.0769C15.3693 12.7638 16.2 10.9584 16.2 9C16.2 5.0967 12.9033 1.8 9 1.8C5.0967 1.8 1.8 5.0967 1.8 9C1.8 10.9584 2.6307 12.7638 3.951 14.0769C4.2943 13.2596 4.87104 12.5617 5.60904 12.0705C6.34704 11.5794 7.21351 11.3168 8.1 11.3157H9.9C10.7865 11.3168 11.653 11.5794 12.391 12.0705ZM5.40001 7.2C5.40001 5.148 6.94801 3.6 9.00001 3.6C11.052 3.6 12.6 5.148 12.6 7.2C12.6 9.252 11.052 10.8 9.00001 10.8C6.94801 10.8 5.40001 9.252 5.40001 7.2ZM7.20001 7.2C7.20001 8.2602 7.93981 9 9.00001 9C10.0602 9 10.8 8.2602 10.8 7.2C10.8 6.1398 10.0602 5.4 9.00001 5.4C7.93981 5.4 7.20001 6.1398 7.20001 7.2Z"
              fill="#9B9B9B"
            />
          </svg>
          <span className="navigation-user__text">Профиль</span>
        </li>
      </ul>
    </nav>
  );
};
